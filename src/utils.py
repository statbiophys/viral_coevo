...
def vtau_lin_mutrate(p):
    return p.r0*((p.beta*(1-p.lambd*p.D_coef)/(p.alpha + p.gamma))**(1/p.M) - 1)**(-1)


def speed_lin_mutrate(p, size, cutoff):
    vt = vtau_lin_mutrate(p)
    s = sel_coef_lin(p, vt)
    d_inv = (s/p.D_coef)**(1/3)
    eps0 = - 2.3381
    r = size/cutoff
...
def size_lin_mutrate(p, cutoff):
    
    vtau = vtau_lin_mutrate(p)
    def _Neq(x, p, cutoff, vtau):
        return (vtau - speed_lin_mutrate(p, x, cutoff)*p.M*p.Nh/x)
...
def lin_from_two_log_points(x, x1, y1, x2, y2):
    return lin_from_two_points(np.log(x), np.log(x1), y1, np.log(x2), y2)

def lin_from_two_loglog_points(x, x1, y1, x2, y2):
    return np.exp(lin_from_two_log_points(x, x1, np.log(y1), x2, np.log(y2)))